...
template <typename T>
class OuterMemory : public benchmark::Fixture {
 public:
  void callKernel(benchmark::State &state) {
    // call kernel
    GEMM5<TPB>(dA, dB, dC, M, N, K);
  }
  ...
  void TearDown(const ::benchmark::State &st) BENCHMARK_OVERRIDE {
    if (!cudabm::Equal<T>(M * N, dC, testC, 1e-2))
      throw std::runtime_error("Value diff occur in outer memory");
...
#define BENCHMARK_GEMM5_OP(name, dType)                                \
  BENCHMARK_TEMPLATE_DEFINE_F(OuterMemory, name, dType)                \
  (benchmark::State & st) {                                            \
    for (auto _ : st) {                                                \
      callKernel(st);                                                  \
    }                                                                  \
  ...
  BENCHMARK_REGISTER_F(OuterMemory, name)                              \
