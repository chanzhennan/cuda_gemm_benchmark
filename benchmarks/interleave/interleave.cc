...
#include "interleave/interleave.cuh"
...
template <typename T>
class Interleave : public benchmark::Fixture {
 public:
...
#define BENCHMARK_GEMM8_OP(name, dType)                                \
  BENCHMARK_TEMPLATE_DEFINE_F(Interleave, name, dType)                 \
  (benchmark::State & st) {                                            \
    for (auto _ : st) {                                                \
      callKernel(st);                                                  \
    }                                                                  \
  ...
  BENCHMARK_REGISTER_F(Interleave, name)                               \
