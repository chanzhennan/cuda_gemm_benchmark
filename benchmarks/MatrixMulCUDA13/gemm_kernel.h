...
struct Gemm {
  ...
  // slice_k = k / w_k
  static constexpr int SLICE_K = TILE_K / SLICES;  // 32 / 4
...
struct GemmKernel : public CoreKernel {
  ...
  static constexpr int kSmemByteSize =
      std::max(kSmemSizeA + kSmemSizeB, kSmemSizeC);
  ...
  void Launch(float* C, const float* A, const float* B, int M, int N,
    ...
    gemm_kernel<GemmType><<<grid_size, block_size>>>(C, A, B, M, N, K);
  }
