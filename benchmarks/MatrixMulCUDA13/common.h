...
static constexpr int OP_M = 64;
static constexpr int OP_N = 32;
...
// // Modified from llm-awq
// https://github.com/mit-han-lab/llm-awq/blob/main/awq/kernels/csrc/quantization/dequantize.cuh
...
//     // Note that the entire sequence only requires 1 shift instruction. This
//     is
//     // thanks to the register packing format and the fact that we force our
//     // integers to be unsigned, and account for this in the fp16
//     subtractions. In
//     // addition, I exploit the fact that sub and fma have the same throughput
//     in
//     // order to convert elt_23 and elt_67 to fp16 without having to shift
//     them to
//     // the bottom bits before hand.

//     // Shift right by 8 to now consider elt_45 and elt_67. Issue first to
//     hide RAW
...
//     // Haotian: subtract {1024, 1024} instead, we do not need to map to [-8,
//     7] static constexpr uint32_t FP16_TOP_MAGIC_NUM = 0x64006400;
...
//     // Convert elt_01
//     asm("sub.f16x2 %0, %1, %2;\n" : "=r"(h[0]) : "r"(h[0]),
//     "r"(FP16_TOP_MAGIC_NUM));
//     // Convert elt_23
//     asm("fma.rn.f16x2 %0, %1, %2, %3;\n" : "=r"(h[1]) : "r"(h[1]),
//     "r"(ONE_SIXTEENTH), "r"(NEG_64));
//     // Convert elt_45
//     asm("sub.f16x2 %0, %1, %2;\n" : "=r"(h[2]) : "r"(h[2]),
//     "r"(FP16_TOP_MAGIC_NUM));
//     // Convert elt_67
...
// __inline__ __device__ uint4 dequantize_s4_to_fp16x2_v2(uint32_t const&
// source) {
//     uint4 result;
...
//     if (0) {  // 1024 & 64
//         asm("lop3.b32 %0, %1, %2, %3, %4;\n" : "=r"(h[0]) : "r"(i4s),
//         "n"(BOT_MASK), "n"(MAGIC_NUM_0), "n"(immLut)); asm("lop3.b32 %0, %1,
//         %2, %3, %4;\n" : "=r"(h[1]) : "r"(i4s), "n"(TOP_MASK),
//         "n"(MAGIC_NUM_1), "n"(immLut)); asm("lop3.b32 %0, %1, %2, %3, %4;\n"
//         : "=r"(h[2]) : "r"(top_i4s), "n"(BOT_MASK), "n"(MAGIC_NUM_0),
//         "n"(immLut)); asm("lop3.b32 %0, %1, %2, %3, %4;\n" : "=r"(h[3]) :
//         "r"(top_i4s), "n"(TOP_MASK), "n"(MAGIC_NUM_1), "n"(immLut));
//         asm("sub.f16x2 %0, %1, %2;\n" : "=r"(h[0]) : "r"(h[0]),
//         "r"(MAGIC_NUM_0)); asm("sub.f16x2 %0, %1, %2;\n" : "=r"(h[1]) :
//         "r"(h[1]), "r"(MAGIC_NUM_1)); asm("sub.f16x2 %0, %1, %2;\n" :
//         "=r"(h[2]) : "r"(h[2]), "r"(MAGIC_NUM_0)); asm("sub.f16x2 %0, %1,
//         %2;\n" : "=r"(h[3]) : "r"(h[3]), "r"(MAGIC_NUM_1));
//     } else {  //  64 only, trade 4 hfma2 with 2 shifts
//         asm("lop3.b32 %0, %1, %2, %3, %4;\n" : "=r"(h[0]) : "r"(i4s),
//         "n"(BOT_MASK), "n"(MAGIC_NUM_2), "n"(immLut)); asm("lop3.b32 %0, %1,
//         %2, %3, %4;\n" : "=r"(h[1]) : "r"(i4s), "n"(TOP_MASK),
//         "n"(MAGIC_NUM_1), "n"(immLut)); asm("lop3.b32 %0, %1, %2, %3, %4;\n"
//         : "=r"(h[2]) : "r"(top_i4s), "n"(BOT_MASK), "n"(MAGIC_NUM_2),
//         "n"(immLut)); asm("lop3.b32 %0, %1, %2, %3, %4;\n" : "=r"(h[3]) :
//         "r"(top_i4s), "n"(TOP_MASK), "n"(MAGIC_NUM_1), "n"(immLut)); h[0] <<=
//         4; h[2] <<= 4;
//         // we don't need to subtract the magic nums because zeros will go
//         through the same dequant function
//         // and carry the same magic constant, the magic num will be canceled
//         out after subtracting zeros
//     }
...
// __inline__ __device__ void ldmatrix_m8n8_x2_b16(uint& d0, uint& d1, uint32_t
// smem_int_ptr) { #if TURBOMIND_ARCH_SM75
...
struct Array {
  ...
  __device__ __host__ constexpr const_reference operator[](
      size_type i) const noexcept {
    return __a[i];
  }

  __device__ __host__ constexpr reference front() noexcept { return *begin(); }
  ...
  __device__ __host__ constexpr pointer data() noexcept { return &__a[0]; }

  __device__ __host__ constexpr const_pointer data() const noexcept {
    return &__a[0];
  }

  __device__ __host__ constexpr iterator begin() noexcept { return data(); }

  __device__ __host__ constexpr const_iterator begin() const noexcept {
    return data();
  }

  __device__ __host__ constexpr iterator end() noexcept { return data() + N; }

  __device__ __host__ constexpr const_iterator end() const noexcept {
    return data() + N;
  }

  __device__ __host__ constexpr std::integral_constant<int, N> size()
      const noexcept {
    return {};
  }
...
struct Shape {
  ...
  constexpr auto m() const noexcept { return get<0>(); }

  constexpr auto n() const noexcept { return get<1>(); }

  constexpr auto k() const noexcept { return get<2>(); }

  constexpr int c() const noexcept { return get<0>(); }

  constexpr int s() const noexcept { return get<1>(); }
