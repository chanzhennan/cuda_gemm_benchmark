...
template <typename T>
class PpBuffer : public benchmark::Fixture {
 public:
  void callKernel(benchmark::State &state) {
    // call kernel
    GEMM6<BLOCKSIZE>(dA, dB, dC, state.range(0), state.range(1),
                     state.range(2));
  }
  ...
  void verify(const ::benchmark::State &st) {
    ...
    if (!cudabm::Equal<T>(st.range(0) * st.range(1), dC, testC, 1e-2))
      throw std::runtime_error("Value diff occur in PpBuffer");
  }
