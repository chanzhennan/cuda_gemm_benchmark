...
template <typename T>
class DefineExtend : public benchmark::Fixture {
 public:
...
#define BENCHMARK_GEMM7_OP(name, dType)                                \
  BENCHMARK_TEMPLATE_DEFINE_F(DefineExtend, name, dType)               \
  (benchmark::State & st) {                                            \
    for (auto _ : st) {                                                \
      callKernel(st);                                                  \
    }                                                                  \
  ...
  BENCHMARK_REGISTER_F(DefineExtend, name)                             \
